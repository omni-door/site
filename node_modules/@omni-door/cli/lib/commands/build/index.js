            ...
            return inquirer_1.default.prompt([
                {
                    name: 'install',
                    type: 'confirm',
                    message: utils_2.logo() + " \u81EA\u52A8\u5B89\u88C5\u6240\u9700\u8981\u7684\u4F9D\u8D56? (Automatic install dependencies?)",
                    default: true
                }
                    ...
                    switch (_a.label) {
                        ...
                        case 1:
                            ...
                            return [2 /*return*/, utils_1.exec([
                                    ...
                                    utils_1.logEmph('构建依赖安装完毕！(The dependencies install completed!)');
                                    return true;
                                }, function (err) {
                                    utils_1.logWarn(err);
                                    utils_1.logWarn('👆 依赖安装发生了错误 (The dependencies install occured some accidents)');
                                    return false;
                                })];
        ...
        function copyReserves(reserves) {
            for (var i = 0, len = reserves.length; i < len; i++) {
                ...
                try {
                    ...
                    utils_1.logWarn("\"" + srcPath + "\" \u662F\u4E00\u4E2A\u65E0\u6548\u7684\u8DEF\u5F84\uFF01(The path of \"" + srcPath + "\" is invaild!)");
                    continue;
                }
                ...
                if (stats.isDirectory()) {
                    ...
                    utils_1.logWarn("\"" + srcPath + "\" \u4E0D\u662F\u6709\u6548\u7684\u6587\u4EF6\u6216\u6587\u4EF6\u5939\u8DEF\u5F84\uFF01(The file or directory path which is \"" + srcPath + "\" cannot be found!)");
                    continue;
                }
        ...
        return __generator(this, function (_k) {
            switch (_k.label) {
                ...
                case 3:
                    ...
                    // bind exit signals
                    utils_2.signal();
                    message = '开始构建！(Building process start!)';
                    utils_1.logTime('项目构建');
                ...
                case 4:
                    ...
                    return [4 /*yield*/, utils_1.exec(['npm test'], function () { return utils_1.logEmph(utils_1.italic('单元测试通过！(The unit test passed!)')); }, handleBuildErr('单元测试失败！(The unit test failed!)'))];
                case 5:
                    _k.sent();
                    _k.label = 6;
                case 6:
                    if (!(verify && eslint)) return [3 /*break*/, 8];
                    return [4 /*yield*/, utils_1.exec(['npm run lint:es'], function () { return utils_1.logEmph(utils_1.italic('eslint校验通过！(The eslint passed!)')); }, handleBuildErr("eslint\u6821\u9A8C\u5931\u8D25\uFF01(The eslint checking failed!) \n \u5C1D\u8BD5\u6267\u884C (try to exec): " + utils_1.underline('npm run lint:es_fix')))];
                case 7:
                    _k.sent();
                    _k.label = 8;
                case 8:
                    if (!(verify && prettier)) return [3 /*break*/, 10];
                    return [4 /*yield*/, utils_1.exec(['npm run lint:prettier'], function () { return utils_1.logEmph(utils_1.italic('prettier校验通过！(The prettier passed!)')); }, handleBuildErr("prettier\u6821\u9A8C\u5931\u8D25\uFF01(The prettier checking failed!) \n \u5C1D\u8BD5\u6267\u884C (try to exec): " + utils_1.underline('npm run lint:prettier_fix')))];
                case 9:
                    _k.sent();
                    _k.label = 10;
                case 10:
                    if (!(verify && stylelint)) return [3 /*break*/, 12];
                    return [4 /*yield*/, utils_1.exec(['npm run lint:style'], function () { return utils_1.logEmph(utils_1.italic('stylelint校验通过！(The stylelint passed!)')); }, handleBuildErr("stylelint\u6821\u9A8C\u5931\u8D25\uFF01(The stylelint checking failed!) \n \u5C1D\u8BD5\u6267\u884C (try to exec): " + utils_1.underline('npm run lint:style_fix')))];
                case 11:
                    _k.sent();
                    _k.label = 12;
                case 12:
                    ...
                    utils_1.logWarn('请先安装 typescript 相关依赖! (Please install typescript first!)');
                ...
                case 15:
                    ...
                    utils_1.logWarn('请先安装 webpack 相关依赖! (Please install webpack first!)');
                ...
                case 18:
                    ...
                    content_webpack = !buildConfig && (type === 'spa-react' || type === 'spa-vue') && webpack_1.default({ ts: typescript, multiOutput: false, srcDir: srcDir, outDir: outDir, configurationPath: configurationPath, pkjFieldName: pkjFieldName, configFileName: configFileName, hash: hash });
                    content_gulp = !buildConfig && type === 'component-react' && gulp_1.default({ srcDir: srcDir, outDir: outDir, esmDir: esmDir });
                    ...
                    utils_1.logWarn('请先安装 rollup 相关依赖! (Please install rollup first!)');
                ...
                case 20:
                    ...
                    return [3 /*break*/, 29];
                case 21:
                    if (!(type === 'spa-react' || type === 'spa-vue')) return [3 /*break*/, 24];
                ...
                case 23:
                    ...
                    return [3 /*break*/, 29];
                case 24:
                    if (!(type === 'component-react')) return [3 /*break*/, 29];
                    tscPath = buildCliPath.tsc;
                ...
                case 26:
                    ...
                    utils_1.logWarn('请先安装 gulp 相关依赖! (Please install gulp first!)');
                ...
                case 30:
                    ...
                    if (type !== 'toolkit') {
                        ...
                        utils_1.spinner.state('start', '项目构建中 (Building, please wait patiently)');
                    }
                                    ...
                                    switch (_d.label) {
                                        ...
                                        case 4:
                                            if (realOutDir_1 && !fs_1.default.existsSync(realOutDir_1)) {
                                                handleBuildErr("\u8F93\u51FA\u7684 " + realOutDir_1 + " \u6587\u4EF6\u4E0D\u5B58\u5728\uFF0C\u6784\u5EFA\u5931\u8D25\uFF01(The output file " + realOutDir_1 + " doesn't exist)")();
                                            }
                                            else {
                                                type !== 'toolkit' && utils_1.spinner.state('stop');
                                                utils_1.logTime('项目构建', true);
                ...
                case 31:
                    ...
                    utils_1.logInfo('开始自动发布！(Start auto release!)');
                    _k.label = 32;
                case 32:
